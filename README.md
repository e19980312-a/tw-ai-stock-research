...
- 可搜尋股票、查看本地研究資料。
- 可連網產生並顯示研究建議。
- 新股票可產生初步研究卡並暫存於 `research_candidates.csv`。
...
私人模式會在候選研究卡顯示「加入正式研究資料庫」按鈕。按下後才會備份 `stocks.csv`、加入目前候選股票、執行 `score_stocks.py` 並更新 `decision_summary.csv`。
...
- `research_candidates.csv`：新股票的初步公司定位、分數與研究決策。
...
搜尋 `stocks.csv` 尚未收錄的股票，例如 `2317` 或 `3081` 時：

1. 系統先查 `decision_summary.csv`。
2. 若沒有正式資料，顯示「連網產生初步研究卡」。
3. `fetch_stock_info.py` 同時更新 `suggested_scores.csv` 與 `research_candidates.csv`。
4. 初步研究卡顯示公司定位、AI 相關性、瓶頸判斷、五項分數、研究決策、風險與資料來源。

公司定位先以關鍵字規則補全，涵蓋 AI 伺服器／ODM、光通訊／CPO、PCB／CCL／ABF、記憶體、被動元件、電源與工業電腦等類型。

若資料不足：

- `ai_relevance` 與 `is_bottleneck` 顯示「未判定」。
- `confidence_level` 降為「低」。
- 風險備註加入「資料不足，需人工確認」。
- 研究決策最多為「續列觀察」或「僅追蹤不投入」，不會輸出積極布局候選。

`research_candidates.csv` 固定欄位：

```text
generated_at, stock_id, stock_name, market, industry_position,
ai_relevance, is_bottleneck, industry_score, growth_score, ai_score,
valuation_score, price_risk_score, total_score, research_decision,
research_signal, signal_strength, research_reason, risk_notes,
research_note, confidence_level, source_urls
```
...
v1.1 新增新股票初步研究建檔、自動公司定位、候選清單顯示與私人模式正式入庫流程。
...
- 本地沒有資料時，可直接執行連網研究。
- 新股票完成連網後會顯示完整初步研究摘要與公司定位，不再只顯示空白卡片。
