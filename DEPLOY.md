...
```bash
git add stocks.csv decision_summary.csv watchlist.csv suggested_scores.csv
...
v1.0.5 公開模式下，「我的清單」已改用個別 session 暫存，不會寫入共用的 `watchlist.csv`。`suggested_scores.csv` 仍是連網研究的暫時輸出，可能因 App 重啟或其他研究請求而更新，不應視為永久資料庫。
